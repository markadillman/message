...
function putTeleMarkerInCanvas(myGroupStr, myContext) {
	...
	putGroupInCanvas(myGroupStr, myContext, 0, 0, 20, 20, 0, 0, 20, 20, true)
}
function drawMarkersOnMap() {
	...
	for (var i = 0; i < teleNumMarkers; i += 1) {
		...
		if (teleMapTileX >= 0 && teleMapTileX <= mapGridWidth && teleMapTileY >= 0 && teleMapTileY <= mapGridHeight) {
			...
			// get in-map coordinates
			teleMapX = teleMapInTileX + (teleMapTileX * (canvasWidth/10));
			...
			teleMarkerDivList[i].style.top = teleMapY.toString() + "px";
...
function teleMarkerSelect(markerID) {
	...
	putTeleMarkerInCanvas(selectedTeleMarkerImg, teleMarkerCanvasList[markerID].getContext("2d"));
...
function doTeleport() {
	...
	var newXCoord = Number(newCoords[0]);
	var newYCoord = Number(newCoords[1]);
	...
	// debug messsage
	if (debugging) {
		console.log("Teleported to coordinate: (" + newXCoord.toString() + 
					", " + newYCoord.toString() + ").");
	}
}
